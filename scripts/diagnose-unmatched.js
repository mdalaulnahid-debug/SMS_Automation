...
const total = db.prepare(`
  ...
  WHERE matched_request_id IS NULL AND (analysis IS NULL OR analysis NOT LIKE '%"ignored":true%')
`).get().n;
...
const range = db.prepare(`
  ...
  FROM sms_inbox WHERE matched_request_id IS NULL AND (analysis IS NULL OR analysis NOT LIKE '%"ignored":true%')
`).get();
...
const last24h = db.prepare(`
  SELECT COUNT(*) AS n FROM sms_inbox
  WHERE matched_request_id IS NULL AND (analysis IS NULL OR analysis NOT LIKE '%"ignored":true%') AND received_at >= datetime('now', '-1 day')
`).get().n;
const last7d = db.prepare(`
  SELECT COUNT(*) AS n FROM sms_inbox
  WHERE matched_request_id IS NULL AND (analysis IS NULL OR analysis NOT LIKE '%"ignored":true%') AND received_at >= datetime('now', '-7 days')
`).get().n;
